...
class AuditProtocol:
    ...
    numerical_backend_sha256: str | None = None
...
def ensure_protocol_lock(path: str | Path, protocol: AuditProtocol) -> dict[str, Any]:
    ...
    desired["rule"] = (
        "Changing a locked detector version, parser version, numerical solver version, numerical backend "
        "fingerprint, threshold, assumption, artifact identity, methodology snapshot, extraction calibration, "
        "or specification space requires a new audit run."
    )
