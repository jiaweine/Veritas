...
6. **Development calibration** — extraction thresholds, parser routing, and promotion policy may be tuned only on train/development families. `ExtractionSplitManifest` mechanically derives the DEVELOPMENT target universe from the exact reviewed gold plus deterministic family lock. Every threshold observation must retain a full benchmark report for exactly those targets, and the frozen threshold commits the SHA-256 of the complete DEVELOPMENT observation set. The selected threshold must be one of the threshold IDs/values committed before TEST.
7. **Untouched TEST** — the TEST families are used only after the extraction/promotion protocol is frozen. The canonical TEST `ExtractionSplitManifest` is derived from the same reviewed gold and split lock and is bound into `ExtractionTestEvaluationLock`. TEST outcomes cannot be used to change thresholds and then be reported as held-out evidence. A case already used for parser development does not become untouched merely because its labels later receive double review.
8. **Release receipt** — `ExtractionEvidenceReleaseReceipt` can be issued only if the reviewed gold belongs to both the precommitted sampling frame and the exact seed-derived review-target universe, including its page/table/row locator identity, and if its deterministic split lock, frozen DEVELOPMENT threshold, TEST seal, TEST evaluation lock, and manifest-bound DEVELOPMENT/TEST coverage–selectivity evidence all validate against the same pre-TEST evidence plan. The receipt remains `production_authorized = false`.

The release workflow is deliberately stricter than having a collection of hashes. It mechanically rejects mixed sampling frames, changed seed bytes, changed seed target-universe semantics, target or locator drift, altered review protocols, split drift, post-hoc threshold grids, changed DEVELOPMENT/TEST manifests, invalid TEST seals, reports whose outcome membership differs from the canonical split target universe, a frozen threshold selected from a different DEVELOPMENT observation set, and selectivity evidence built from another gold manifest or split lock.
...
Coverage–selectivity curves are not accepted as free-floating aggregate arrays. Each published curve is reconstructed from `ExtractionSelectivityEvidence`: one canonical split manifest plus a complete threshold-observation set whose benchmark reports retain the exact per-target outcome membership. DEVELOPMENT evidence must hash to the same observation set that selected the frozen threshold, and both DEVELOPMENT and TEST evidence must preserve the complete precommitted threshold `ID -> value` mapping.
