...
def _positive_pdf(index: int, rng: random.Random, *, corrupt: bool = False) -> tuple[bytes, str]:
    ...
    variables = ("Treatment assignment", "Policy exposure", "Program participation", "Income shock")
    variable = variables[index % len(variables)]
    base_xs = (54.0, 190.0, 286.0, 390.0, 470.0)
    shift = rng.uniform(-8.0, 12.0)
    jitter = (0.0, rng.uniform(-5.0, 5.0), rng.uniform(-4.0, 4.0), rng.uniform(-3.0, 3.0), rng.uniform(-3.0, 3.0))
    ...
    caption = rng.choice(("Table 4. Regression estimates", "TABLE 2 Logistic regression", "Table A1. Main model"))
    _insert_table(
        page,
        y=y,
        xs=xs,
    ...
    if index % 4 == 0:
        # Same-y right-column prose deliberately pressures the last-column geometry bound.
        page.insert_text((555, y), "Notes", fontsize=6.5)
        page.insert_text((555, y + 28), "robust", fontsize=6.5)
    if index % 5 == 0:
        # A second unrelated table-like block below the target checks that matching remains local.
        _insert_table(
            page,
            y=y + 235,
            xs=xs,
...
def _negative_pdf(index: int) -> tuple[bytes, str]:
    ...
    if mode == 0:
        ...
        # Caption exists, but the target is far outside the local table region.
        _insert_table(
            page,
            y=y,
            xs=xs,
        ...
        # A real caption but incomplete statistical semantics: no z/t column.
        ...
        # Explicit regression header, but requested variable never appears.
        _insert_table(
            page,
            y=y,
            xs=xs,
...
def _synthetic_geometry_gate() -> ConformalExtractionGate:
    # Synthetic-only calibration for stress evaluation. This is not production certification.
...
def _audit(pdf: bytes, variable: str, *, gate: ConformalExtractionGate) -> tuple[bool, bool, bool, float]:
    ...
    exact = dual and all(len({candidate.normalized_value for candidate in bundle.field_candidates[key]}) == 1 for key in fields)
    if not exact:
        return dual, False, False, elapsed_ms
    ...
    return dual, report.hard_audit_ready, hard, elapsed_ms
