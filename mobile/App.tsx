...
type Tab = "overview" | "audits" | "findings";
...
export default function App() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setOverview(o); setAudits(a); setFindings(f);
      if (activeAudit) {
  ...
  const openAudit = async (id: string) => {
    ...
    } catch (error) { Alert.alert("Unable to open audit", String(error)); }
  };
  ...
  const uploadPdf = async () => {
    ...
    try {
      ...
      await load();
      setActiveAudit(audit);
    } catch (error) { Alert.alert("Upload failed", String(error)); }
  ...
  const send = async () => {
    ...
    try {
      ...
      setActiveAudit(next);
      await load();
    } catch (error) { Alert.alert("Audit command failed", String(error)); }
    finally { setSending(false); }
  };

  if (activeAudit) {
    return <SafeAreaView style={styles.app}><StatusBar barStyle="dark-content" /><AuditDetail audit={activeAudit} onBack={() => setActiveAudit(null)} message={message} setMessage={setMessage} send={send} sending={sending} /></SafeAreaView>;
  }

  return <SafeAreaView style={styles.app}>
    <StatusBar barStyle="dark-content" />
    <View style={styles.header}><View><Text style={styles.brand}>Veritas</Text><Text style={styles.brandSub}>Research audit</Text></View><Pressable style={styles.uploadButton} onPress={uploadPdf}><Text style={styles.uploadButtonText}>＋ Paper</Text></Pressable></View>
    <View style={styles.body}>
      {tab === "overview" && <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} contentContainerStyle={styles.scroll}>
        <Text style={styles.eyebrow}>RESEARCH AUDIT COCKPIT</Text><Text style={styles.title}>Evidence in view.</Text><Text style={styles.subtitle}>Inspect real verification coverage, contradiction findings, and paper provenance from the same local-first harness.</Text>
    ...
    <View style={styles.tabbar}>{(["overview","audits","findings"] as Tab[]).map((item) => <Pressable key={item} onPress={() => setTab(item)} style={styles.tab}><Text style={[styles.tabIcon, tab === item && styles.tabActive]}>{item === "overview" ? "⌂" : item === "audits" ? "▤" : "◇"}</Text><Text style={[styles.tabLabel, tab === item && styles.tabActive]}>{item === "overview" ? "Overview" : item === "audits" ? "Audits" : "Findings"}</Text></Pressable>)}</View>
  </SafeAreaView>;
}
...
function AuditDetail({ audit, onBack, message, setMessage, send, sending }: { audit: Audit; onBack: () => void; message: string; setMessage: (v:string)=>void; send:()=>void; sending:boolean }) {
...
function MiniMetric({ label, value }: { label:string; value:number }) { return <View style={styles.miniMetric}><Text style={styles.metricLabel}>{label}</Text><Text style={styles.miniMetricValue}>{value}</Text></View>; }
